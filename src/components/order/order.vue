                ...
                <li v-for="item in goods" class="food-list food-list-hook">
                    ...
                    <ul>
                        <li @click="selectFood(food,$event)" v-for="food in item.foods" class="food-item border-1px">
                            <div class="icon">
                                <img width="57" height="57" :src="food.icon" alt="food_icon">
                            </div>
                            <div class="content">
...
export default {
    ...
    computed: {
        ...
        oldGoods() {
            return this.$store.state.oldGood;
        },
        currentIndex() {
    ...
    created() {
        ...
        this.$root.eventHub.$on('cart_add', this._drop);
        if (this._isEmpty(this.oldGoods)) {
    ...
    beforeDestroy() {
        this.$root.eventHub.$off('cart_add', this._drop);
    },
    methods: {
    ...
    beforeRouteLeave(to, from, next) {
        ...
        // 可以访问组件实例 `this`
        if (to.name === 'payment') {
            this.$store.commit('_push', this.seller);
            this.$store.commit('_update', this.selectFoods);
            this.$store.commit('_add', this.goods);
        }
        next();
    }
};
